...
# 1 "mcc_generated_files/LoRaWAN/lorawan.c"
...
# 52 "C:\Program Files (x86)\Microchip\xc8\v2.05\pic\include\pic18lf46k22.h"
...
# 60 "mcc_generated_files/LoRaWAN/lorawan.h"
typedef enum
{
OK = 0,
NETWORK_NOT_JOINED ,
...
# 47 "mcc_generated_files/LoRaWAN/lorawan_aes.h"
void AESEncodeLoRa(unsigned char* block, unsigned char* key);

# 47 "mcc_generated_files/LoRaWAN/lorawan_aes_cmac.h"
void AESCmac(uint8_t* key, uint8_t* output, uint8_t* input, uint8_t size);

# 110 "mcc_generated_files/LoRaWAN/lorawan_private.h"
typedef union
{
uint8_t value;
struct
...
{
uint8_t receivedCid;
unsigned channelMaskAck :1;
unsigned dataRateAck :1;
unsigned powerAck :1;
unsigned channelAck :1;
unsigned dataRateReceiveWindowAck :1;
unsigned rx1DROffestAck :1;
unsigned dataRateRangeAck :1;
unsigned channelFrequencyAck :1;
} LorawanCommands_t;
...
LorawanError_t SelectChannelForTransmission (bool transmissionType);
...
# 55 "mcc_generated_files/LoRaWAN/radio_driver_SX1276.h"
typedef enum
{
MODULATION_FSK = 0,
MODULATION_LORA,
...
void SwTimersClear(uint8_t timerId);
void SwClearAllTimers(void);

void TMR_OverrideRemaining(uint32_t ticksRemaining);
uint32_t TMR_GetDeltaTime(void);
uint16_t TMR_SwapTimer(uint16_t timerVal);

# 58 "mcc_generated_files/LoRaWAN/interrupt_manager_lora_addons.h"
...
# 85 "mcc_generated_files/LoRaWAN/lorawan_eu.h"
typedef union
{
...
struct
{
unsigned ackRequiredFromNextDownlinkMessage:1;
unsigned ackRequiredFromNextUplinkMessage:1;
unsigned joining: 1;
unsigned fPending:1;
...
LorawanCommands_t macCommands[16];
...
# 52 "mcc_generated_files/LoRaWAN/lorawan.c"
static const uint8_t macEndDevCmdReplyLen[] = {1, 2, 1, 2, 3, 2, 1};

LoRa_t loRa;

uint8_t macBuffer[271];
uint8_t radioBuffer[271];
static uint8_t aesBuffer[16];
...
if (loRa.macStatus.macPause == 1)
{
return MAC_PAUSED;
}

if (loRa.macStatus.silentImmediately == 1)
{
...
SwTimerSetTimeout(loRa.abpJoinTimerId, ( (((uint32_t)(20)) << (12)) / 125UL ));
...
if (loRa.fCntUp.value == (4294967295UL))
{
...
loRa.lorawanMacStatus.synchronization = 1;
...
void LORAWAN_GetMcastApplicationSessionKey (uint8_t *mcastApplicationSessionKey)
{
if (mcastApplicationSessionKey != (0))
{
...
loRa.adrAckCnt = 0;
loRa.lorawanMacStatus.adrAckRequest = 0;
}
...
RADIO_SetWatchdogTimeout(((uint32_t)15000));
RADIO_ReceiveStop();
}
}
...
loRa.macStatus.channelsModified = 0;
loRa.macStatus.txPowerModified = 0;
loRa.macStatus.nbRepModified = 0;
loRa.macStatus.prescalerModified = 0;
loRa.macStatus.secondReceiveWindowModified = 0;
loRa.macStatus.rxTimingSetup = 0;
...
{
timeToPause = ((SwTimerReadValue (loRa.joinAccept1TimerId)) < 0x00DFAC1FUL ? ( (((uint32_t)(SwTimerReadValue (loRa.joinAccept1TimerId))) * 125UL) >> (12) ) : ( (((uint32_t)(SwTimerReadValue (loRa.joinAccept1TimerId))) >> (12)) * 125UL ) );
}
else if (loRa.macStatus.networkJoined == 1)
{
timeToPause = ((SwTimerReadValue (loRa.receiveWindow1TimerId)) < 0x00DFAC1FUL ? ( (((uint32_t)(SwTimerReadValue (loRa.receiveWindow1TimerId))) * 125UL) >> (12) ) : ( (((uint32_t)(SwTimerReadValue (loRa.receiveWindow1TimerId))) >> (12)) * 125UL ) );
}
} break;
...
timeToPause = ((timeToPause) < 0x00DFAC1FUL ? ( (((uint32_t)(timeToPause)) * 125UL) >> (12) ) : ( (((uint32_t)(timeToPause)) >> (12)) * 125UL ) );
} break;
...
if (timeToPause >= 200)
{
timeToPause = timeToPause - 50;
loRa.macStatus.macPause = 1;
}
else
{
timeToPause = 0;
...
loRa.macCommands[iCtr].receivedCid = 0xFF;
loRa.crtMacCmdIndex --;
}
}
}
...
{
loRa.macStatus.linkCheck = 1;


if (loRa.macStatus.networkJoined == 1)
{
SwTimerSetTimeout(loRa.linkCheckTimerId, ((loRa.periodForLinkCheck) < 0x000FFFFFUL ? ( (((uint32_t)(loRa.periodForLinkCheck)) << (12)) / 125UL ) : ( (((uint32_t)(loRa.periodForLinkCheck)) / 125UL) << (12) ) ));
...
void LORAWAN_ForceEnable (void)
{
loRa.macStatus.silentImmediately = 0;
}
...
{
loRa.receiveWindow1Parameters.dataRate = 0;
}
...
if (rxPayload.RxAppData != (0))
{
rxPayload.RxAppData ((0), 0, MAC_NOT_OK);
}
}
}
else
...
if ((loRa.macStatus.macPause == 0) && (loRa.macStatus.linkCheck == 1))
{
...
{
maximumPacketSize = LORAWAN_GetMaxPayloadSize () + 13;
...
if ( ( (loRa.counterRepetitionsConfirmedUplink % 2) == 0) && (loRa.currentDataRate != 0) && (loRa.counterRepetitionsConfirmedUplink != (loRa.maxRepetitionsConfirmedUplink + 1) ) )
{
...
if (rxPayload.RxAppData != (0))
{
rxPayload.RxAppData((0), 0, INVALID_BUFFER_LEN);
}
}
}
else if ( (loRa.counterRepetitionsConfirmedUplink > loRa.maxRepetitionsConfirmedUplink) && (loRa.macStatus.macPause == 0) )
{
ResetParametersForConfirmedTransmission ();
if (rxPayload.RxAppData != (0))
{
...
loRa.lorawanMacStatus.fPending = 0;
}
...
SwTimerSetTimeout(loRa.ackTimeoutTimerId, ( (((uint32_t)(loRa.protocolParameters.ackTimeout)) << (12)) / 125UL ));
...
{
loRa.lorawanMacStatus.joining = 0;
loRa.macStatus.networkJoined = 1;
loRa.macStatus.macState = IDLE;
...
if (rxPayload.RxJoinResponse != (0))
{
rxPayload.RxJoinResponse(1);
}
}
...
{
if ( (Channels[i].dataRange.min < loRa.minDataRate) && (Channels[i].status == 1) )
{
loRa.minDataRate = Channels[i].dataRange.min;
}
if ( (Channels[i].dataRange.max > loRa.maxDataRate) && (Channels[i].status == 1) )
{
...
loRa.lorawanMacStatus.ackRequiredFromNextDownlinkMessage = 0;
}
...
loRa.activationParameters.deviceAddress.value = joinAccept->members.deviceAddress.value;

UpdateReceiveDelays (joinAccept->members.rxDelay & 0x0F);
...
if ( (CLASS_C == loRa.deviceClass) && (hdr->members.devAddr.value == loRa.activationParameters.mcastDeviceAddress.value) && (1 == loRa.macStatus.mcastEnable) )
{

if (0 == CheckMcastFlags(hdr))
{
loRa.macStatus.macState = CLASS_C_RX2_2_OPEN;
LORAWAN_EnterContinuousReceive();
...
AssembleEncryptionBlock (1, hdr->members.fCnt, bufferLength - sizeof (computedMic), 0x49, 1);
...
if (rxPayload.RxAppData != (0))
{
rxPayload.RxAppData ((0), 0, MCAST_RE_KEYING_NEEDED);
}
...
{
loRa.fMcastCntDown.members.valueLow = hdr->members.fCnt;
}
}
else
{
...
if (loRa.fMcastCntDown.value == (4294967295UL))
{
if (rxPayload.RxAppData != (0))
{
rxPayload.RxAppData ((0), 0, MCAST_RE_KEYING_NEEDED);
...
if ( (sizeof(extractedMic) + hdr->members.fCtrl.fOptsLen + 8) != bufferLength)
{
fPort = *(buffer++);

frmPayloadLength = bufferLength - 8 - sizeof (extractedMic);
bufferIndex = 16 + 9;

EncryptFRMPayload (buffer, frmPayloadLength - 1, 1, loRa.fMcastCntDown.value, loRa.activationParameters.mcastApplicationSessionKey, bufferIndex, radioBuffer, 1);
packet = buffer - 1;
}
else
{
...
AssembleEncryptionBlock (1, hdr->members.fCnt, bufferLength - sizeof (computedMic), 0x49, 0);
memcpy (&radioBuffer[0], aesBuffer, sizeof (aesBuffer));
memcpy (&radioBuffer[16], buffer, bufferLength-sizeof(computedMic));
...
if (loRa.fCntDown.value == (4294967295UL))
{
...
frmPayloadLength = bufferLength - 8 - hdr->members.fCtrl.fOptsLen - sizeof (extractedMic);
bufferIndex = 16 + 8 + hdr->members.fCtrl.fOptsLen + sizeof (fPort);

if (fPort != 0)
{
EncryptFRMPayload (buffer, frmPayloadLength - 1, 1, loRa.fCntDown.value, loRa.activationParameters.applicationSessionKey, bufferIndex, radioBuffer, 0);
packet = buffer - 1;
}
else
{

if(bufferLength > (13 + hdr->members.fCtrl.fOptsLen))
{

EncryptFRMPayload (buffer, frmPayloadLength - 1, 1, loRa.fCntDown.value, loRa.activationParameters.networkSessionKey, bufferIndex, radioBuffer, 0);
...
if ( loRa.lorawanMacStatus.ackRequiredFromNextDownlinkMessage == 1 )
{
if (hdr->members.fCtrl.ack == 1)
{
loRa.lorawanMacStatus.ackRequiredFromNextDownlinkMessage = 0;
loRa.macStatus.macState = IDLE;
if (rxPayload.RxAppData != (0))
...
loRa.macStatus.rxDone = 0;
if ( (loRa.macStatus.automaticReply == 1) && (loRa.lorawanMacStatus.synchronization == 0) && ( (loRa.lorawanMacStatus.ackRequiredFromNextUplinkMessage == 1) || (loRa.lorawanMacStatus.fPending == 1) ) )
{
...
uint8_t *ptr;
ptr = buffer;
while ( (ptr < ( buffer + fOptsLen )) && (done == 0) )
{
...
loRa.macCommands[loRa.crtMacCmdIndex].receivedCid = 0xFF;
} break;
...
if((loRa.macCommands[loRa.crtMacCmdIndex].receivedCid != 0xFF) &&
(loRa.crtMacCmdIndex < 16))
{
loRa.crtMacCmdIndex ++;
}
}
...
loRa.macStatus.rxTimingSetup = 1;
...
memset (&macBuffer[0], 0, sizeof (macBuffer) );
...
if (loRa.macStatus.adr == 1)
{
fCtrl.adr = 1;
if(loRa.currentDataRate > loRa.minDataRate)
{
fCtrl.adrAckReq = 1;
loRa.lorawanMacStatus.adrAckRequest = 1;
loRa.adrAckCnt ++;
...
if (loRa.lorawanMacStatus.ackRequiredFromNextUplinkMessage == 1)
{
fCtrl.ack = 1;
loRa.lorawanMacStatus.ackRequiredFromNextUplinkMessage = 0;
}
...
macBuffer[bufferIndex++] = port;

if (bufferLength != 0)
{
memcpy (&macBuffer[bufferIndex], buffer, bufferLength);
EncryptFRMPayload (buffer, bufferLength, 0, loRa.fCntUp.value, loRa.activationParameters.applicationSessionKey, bufferIndex, macBuffer, 0);
...
EncryptFRMPayload (radioBuffer, macCmdIdx, 0, loRa.fCntUp.value, loRa.activationParameters.networkSessionKey, bufferIndex, macBuffer, 0);
bufferIndex = bufferIndex + macCmdIdx;
}

AssembleEncryptionBlock (0, loRa.fCntUp.value, bufferIndex - 16, 0x49, 0);
...
mhdr.bits.mType = FRAME_TYPE_JOIN_REQ;
mhdr.bits.major = 0;
mhdr.bits.rfu = 0;
...
{
if((bIncludeInFopts) && (loRa.macCommands[i].receivedCid != 0xFF))
{
if((bufferIndex - (*pBufferIndex) + macEndDevCmdReplyLen[loRa.macCommands[i].receivedCid - 2]) > 0x0F)
{
break;
}
}
...
{
macBuffer[bufferIndex] |= 0x01;
}

if (loRa.macCommands[i].dataRateAck == 1)
{
macBuffer[bufferIndex] |= 0x02;
}

if (loRa.macCommands[i].powerAck == 1)
{
macBuffer[bufferIndex] |= 0x04;
}
bufferIndex ++;
}
break;
...
{
macBuffer[bufferIndex++] = 0x20;
}
else
{
macBuffer[bufferIndex++] = ((uint8_t)RADIO_GetPacketSnr() & 0x3F);
}
}
break;
...
for ( i = 0; i < loRa.maxChannels; i++ )
{
if ( (dataRate >= Channels[i].dataRange.min) && (dataRate <= Channels[i].dataRange.max ) && ( Channels[i].status == 1 ) )
{
found = 1;
break;
}
}
if ( (found == 0) && (dataRate > loRa.minDataRate) )
{
dataRate = dataRate - 1;
}
}
...
if ((loRa.deviceClass == CLASS_A) && (rxPayload.RxAppData != (0)))
{
loRa.lorawanMacStatus.synchronization = 0;
rxPayload.RxAppData ((0), 0, MAC_NOT_OK);
}
...
loRa.macStatus.networkJoined = 0;
loRa.lorawanMacStatus.joining = 1;
...
memset (&sessionKey[index], 0, sizeof(aesBuffer));
index ++;

memcpy(&sessionKey[index], appNonce, 3);
index = index + 3;

memcpy(&sessionKey[index], networkId, 3);
index = index + 3;
...
loRa.activationParameters.applicationSessionKey[0] = 0x02;
...
loRa.activationParameters.networkSessionKey[0] = 0x01;
...
static void CheckFlags (Hdr_t* hdr)
{
if (hdr->members.fCtrl.adr == 1)
{
loRa.macStatus.adr = 1;
}

if (hdr->members.fCtrl.fPending == 1)
{
loRa.lorawanMacStatus.fPending = 1;
}

if (hdr->members.fCtrl.adrAckReq == 1)
{
loRa.lorawanMacStatus.adrAckRequest = 1;
}
...
{
loRa.lorawanMacStatus.ackRequiredFromNextUplinkMessage = 1;
}
}
...
for (i = 0; i < loRa.crtMacCmdIndex; i++)
{
if(loRa.macCommands[i].receivedCid != 0xFF)
{
if((macCommandLength + macEndDevCmdReplyLen[loRa.macCommands[i].receivedCid - 2]) <= 0x0F)
{
...
AESCmac(key, aesBuffer, buffer, bufferLength);
...
for (j = 0; j < 16; j++)
{
bufferToBeEncrypted[macBufferIndex++] = aesBuffer[j] ^ buffer[16*(i-1) + j];
}
}
...
{
bufferToBeEncrypted[macBufferIndex++] = aesBuffer[j] ^ buffer[(16*k) + j];
}
}
}
...
if ((localDioStatus & 0x01) != 0)
{
RADIO_DIO0();
}
if ((localDioStatus & 0x02) != 0)
{
RADIO_DIO1();
}
if ((localDioStatus & 0x04) != 0)
{
RADIO_DIO2();
}
if ((localDioStatus & 0x08) != 0)
{
RADIO_DIO3();
}
if ((localDioStatus & 0x10) != 0)
{
RADIO_DIO4();
}
if ((localDioStatus & 0x20) != 0)
{
RADIO_DIO5();
}
}
