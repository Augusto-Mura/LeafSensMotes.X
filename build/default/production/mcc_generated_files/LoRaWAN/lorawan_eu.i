...
# 1 "mcc_generated_files/LoRaWAN/lorawan_eu.c"
...
const uint8_t maxPayloadSize[8] = {51, 51, 51, 115, 242, 242, 242, 56};
...
{868500000, 1, { ( ( 5 << (4) ) | 0 ) }, 99, 0, 1, 0xFF},
{868300000, 1, { ( ( 5 << (4) ) | 0 ) }, 99, 0, 1, 0xFF},
{868100000, 1, { ( ( 5 << (4) ) | 0 ) }, 99, 0, 1, 0xFF},
{869525000, 1, { ( ( 5 << (4) ) | 0 ) }, 99, 0, 1, 0xFF},
};

static const ChannelParams_t DefaultChannels433[] = {
{433175000, 1, { ( ( 5 << (4) ) | 0 ) }, 302, 0, 1, 0xFF},
{433375000, 1, { ( ( 5 << (4) ) | 0 ) }, 302, 0, 1, 0xFF},
{433575000, 1, { ( ( 5 << (4) ) | 0 ) }, 302, 0, 1, 0xFF},
};
...
RADIO_Init(&radioBuffer[16], (868100000));
...
loRa.receiveWindow2Parameters.dataRate = (0);
loRa.receiveWindow2Parameters.frequency = (869525000);
}
else
{
RADIO_Init(&radioBuffer[16], (433300000));

InitDefault433Channels ();

loRa.receiveWindow2Parameters.dataRate = (0);
loRa.receiveWindow2Parameters.frequency = (434665000);
}

loRa.txPower = 1;

loRa.currentDataRate = 0;

UpdateMinMaxChDataRate ();


loRa.macKeys.value = 0;
memset (&loRa.activationParameters, 0, sizeof(loRa.activationParameters));


loRa.protocolParameters.receiveDelay1 = 1000UL;
loRa.protocolParameters.receiveDelay2 = 2000UL;
loRa.protocolParameters.joinAcceptDelay1 = 5000UL;
loRa.protocolParameters.joinAcceptDelay2 = 6000UL;
loRa.protocolParameters.ackTimeout = 2000;
loRa.protocolParameters.adrAckDelay = 32;
loRa.protocolParameters.adrAckLimit = 64;
loRa.protocolParameters.maxFcntGap = 16384;
loRa.protocolParameters.maxMultiFcntGap = 16384;

LORAWAN_LinkCheckConfigure (0);
}
...
if ( (ValidateChannelId (channelId, 1) != OK) || (ValidateDataRange (dataRangeNew) != OK) )
{
result = INVALID_PARAMETER;
}
else
...
if (ValidateChannelId (channelId, 1) == OK)
{
...
{
if ( (Channels[channelId].parametersDefined & (0X01 | 0X02 | 0x04) ) == (0X01 | 0X02 | 0x04) )
{
...
bool found = 0;
uint8_t i;
uint32_t delta = 0, minim = (4294967295UL), ticks;
...
{
SwTimerSetTimeout(loRa.joinAccept1TimerId, ( (((uint32_t)(loRa.protocolParameters.joinAcceptDelay1 + rxWindowOffset[loRa.receiveWindow1Parameters.dataRate])) << (12)) / 125UL ));
SwTimerSetTimeout(loRa.joinAccept2TimerId, ( (((uint32_t)(loRa.protocolParameters.joinAcceptDelay2 + rxWindowOffset[loRa.receiveWindow2Parameters.dataRate])) << (12)) / 125UL ));
SwTimerStart(loRa.joinAccept1TimerId);
SwTimerStart(loRa.joinAccept2TimerId);

Channels[i].channelTimer = ((uint32_t)timeOnAir) * (((uint32_t)3029 + 1) * ((uint32_t)loRa.prescaler) - 1);
}
else
{
SwTimerSetTimeout(loRa.receiveWindow1TimerId, ( (((uint32_t)(loRa.protocolParameters.receiveDelay1 + rxWindowOffset[loRa.receiveWindow1Parameters.dataRate])) << (12)) / 125UL ));
SwTimerSetTimeout(loRa.receiveWindow2TimerId, ( (((uint32_t)(loRa.protocolParameters.receiveDelay2 + rxWindowOffset[loRa.receiveWindow2Parameters.dataRate])) << (12)) / 125UL ));
...
delta = loRa.lastTimerValue - ((ticks) < 0x00DFAC1FUL ? ( (((uint32_t)(ticks)) * 125UL) >> (12) ) : ( (((uint32_t)(ticks)) >> (12)) * 125UL ) );
}

for (i=0; i < 16; i++)
{
if ((Channels[i].status == 1) && ( Channels[i].channelTimer != 0 ))
{
...
SwTimerSetTimeout (loRa.dutyCycleTimerId, ((minim) < 0x000FFFFFUL ? ( (((uint32_t)(minim)) << (12)) / 125UL ) : ( (((uint32_t)(minim)) / 125UL) << (12) ) ));
...
{
if ((RADIO_GetStatus() & (1 << (4))) != 0)
{

rxPayload.RxAppData( (0), 0, RADIO_NOT_OK );
}
else
{
...
{
rxPayload.RxAppData( (0), 0, RADIO_OK );
}
}
}
}
...
{
if ( (Channels[i].status == 1) && (Channels[i].channelTimer != 0) && (Channels[i].channelTimer <= minim) && (loRa.currentDataRate >= Channels[i].dataRange.min) && (loRa.currentDataRate <= Channels[i].dataRange.max) )
{
minim = Channels[i].channelTimer;
}
}
SwTimerSetTimeout (loRa.unconfirmedRetransmisionTimerId, ( (((uint32_t)(minim + 50)) << (12)) / 125UL ) );
...
if (rxPayload.RxAppData != (0))
{
rxPayload.RxAppData((0), 0, MAC_OK);
}
}
}
}
...
if (maxDCycle < 15)
{
loRa.prescaler = 1 << maxDCycle;
loRa.macStatus.prescalerModified = 1;
}

if (maxDCycle == 255)
{
loRa.macStatus.silentImmediately = 1;
}
...
if (1 == loRa.macStatus.adr)
{
if ( (ValidateChannelMaskCntl(redundancy->chMaskCntl) == OK) && (ValidateChannelMask(channelMask) == OK) )
{
...
loRa.macStatus.txPowerModified = 1;
...
if (ValidateChannelId (channelIndex, 0) == OK)
{
...
UpdateDutyCycle (channelIndex, 302);
UpdateChannelIdStatus (channelIndex, 1);
}
else
{
LORAWAN_SetChannelIdStatus (channelIndex, 0);
}
}
else
{
...
UpdateDutyCycle (channelIndex + 16, 302);
UpdateChannelIdStatus (channelIndex + 16, 1);
}
else
{
LORAWAN_SetChannelIdStatus (channelIndex + 16, 0);
}
}

loRa.macStatus.channelsModified = 1;
}
return ptr;
}
...
randomNumber = Random (maxChannels) + 1;
...
{
if ( ( Channels[i].status == 1 ) && ( Channels[i].channelTimer == 0 ) && ( loRa.currentDataRate >= Channels[i].dataRange.min ) && ( loRa.currentDataRate <= Channels[i].dataRange.max ) )
{
...
Channels[i+channelIndex].dataRange.max = 5;
Channels[i+channelIndex].dataRange.min = 0;
Channels[i+channelIndex].dutyCycle = 999;
Channels[i+channelIndex].parametersDefined = 0xFF;
LORAWAN_SetChannelIdStatus(i+channelIndex, 1);
loRa.macStatus.channelsModified = 1;
}
}
else
{
LORAWAN_SetChannelIdStatus(i+channelIndex, 0);
}
}
...
loRa.macStatus.macState = RETRANSMISSION_DELAY;
SwTimerSetTimeout (loRa.automaticReplyTimerId, ( (((uint32_t)(minim)) << (12)) / 125UL ) );
...
result = SearchAvailableChannel (16, transmissionType, &channelIndex);
...
Channels[i].dutyCycle = (65535U);
Channels[i].dataRange.value = (255);
}
}
...
Channels[channelId].parametersDefined |= 0X02;
for (i=0; i < loRa.maxChannels; i++)
{
...
if(Channels[channelId].status == 0)
{
...
{
if ( (frequencyNew > 870000000) || (frequencyNew < 863000000) )
{
result = INVALID_PARAMETER ;
}
}
...
{
if ( (frequencyNew > 434790000) || (frequencyNew < 433050000) )
{
result = INVALID_PARAMETER ;
}
}
...
dataRateMin = dataRangeNew & 0x0F;
dataRateMax = (dataRangeNew & 0xF0) >> (4);
...
if ( (channelId >= 16) || ( (allowedForDefaultChannels == 0) && (channelId < 3) ) )
{
result = INVALID_PARAMETER ;
}
...
static void EnableChannels (uint16_t channelMask, uint8_t channelMaskCntl)
{
EnableChannels1 (channelMask, channelMaskCntl, 0, 16);
}

static void UpdateFrequency (uint8_t channelId, uint32_t frequencyNew )
{
Channels[channelId].frequency = frequencyNew;
Channels[channelId].parametersDefined |= 0X01;
}

static void UpdateDutyCycle (uint8_t channelId, uint16_t dutyCycleNew)
{
Channels[channelId].dutyCycle = dutyCycleNew;
Channels[channelId].parametersDefined |= 0x04;
}
...
for (i = 0; i < 16; i++)
{
if ( ( (channelMask & (1 << (0))) == (1 << (0))) && ( (Channels[i].parametersDefined & (0X01 | 0X02 | 0x04) ) != (0X01 | 0X02 | 0x04) ) )
{
return INVALID_PARAMETER;
}
else
...
for ( i = channelIndexMin; i < channelIndexMax; i++ )
{
if ( channelMask & (1 << (0)) == (1 << (0)))
{
UpdateChannelIdStatus (i, 1);
}
else
{
UpdateChannelIdStatus (i, 0);
}
channelMask = channelMask >> (1);
}
}
